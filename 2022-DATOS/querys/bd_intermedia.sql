...
CREATE TABLE `tbl_cpe_cabecera_inter` (
  ...
  `TXT_DMCL_FISC_EMIS` varchar(100) DEFAULT NULL,
  ...
  `TXT_DMCL_FISC_RECEP` varchar(100) DEFAULT NULL,
  ...
  `DATO_EXTRA_1` varchar(50) DEFAULT NULL,
  `DATO_EXTRA_2` varchar(50) DEFAULT NULL,
  `DATO_EXTRA_3` varchar(50) DEFAULT NULL,
  `DATO_EXTRA_4` varchar(50) DEFAULT NULL,
...
CREATE TABLE `tbl_retenciones` (
  ...
  `RETENCION_CODIGO` varchar(6) COLLATE utf8_bin DEFAULT NULL,
  `RETENCION_FACTOR` varchar(10) COLLATE utf8_bin DEFAULT NULL,
...
BEGIN
	declare cpeCab int(11) default 0;
	declare cpeDet int(11) default 0;

	select 
		NUM_CPE_INTERMEDIA into cpeCab 
    from tbl_cpe_cabecera_inter 
	where 
		COD_ESTADO_DOC = '03' 
		and COD_ESTADO_SUNAT = '101'
		and TXT_FLAGCARGA = '1'
        and date_add(FEC_RECP_DOC1, interval 7 minute) < now()
		limit 1;
	
    if cpeCab not in ('',null) then
    
		select 
			count(NUM_CPE_INTERMEDIA) into cpeDet 
		from tbl_cpe_det_cabecera_inter 
		where NUM_CPE_INTERMEDIA = cpeCab;

		if cpeDet = 0 then
			set sql_safe_updates = 0;
			delete from tbl_cpe_cabecera_inter where NUM_CPE_INTERMEDIA = cpeCab; 
			select concat('NumCpe cabecera borrada: ',cpeCab);
...
BEGIN
    ...
    from tbl_retenciones where NUM_CPE_INTERMEDIA = pnumcpe;create table tbl_retenciones
(
...
-- Dump completed on 2021-09-23 15:00:06
